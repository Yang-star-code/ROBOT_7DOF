...
while iterations <= maxIterations && failedAttempts<=maxFailedAttempts  % loop to grow RRTs
    % 生成样本点
    if rand < randfactor
        sample=[rand*3600, rand(1,6).* 360-180]; % 随机采样，有利于找到解，但如果完全由采样点来寻解会影响求解速度
    else
        sample=goal; % 这个可以向目标点拓展，可以加快收敛速度
    end
...
%     failedAttempts=failedAttempts+1;
    ...
    % 检查新点与所有点的距离是否过小
    [A, I]=min( distanceCost(RRTree(:,1:7),newPoint) ,[],1); % check if new node is not already pre-existing in the tree
